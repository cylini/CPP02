...
{
	return (a < b ? a : b); // retourne le plus petit des deux objets
}
...
{
	return (a > b ? a : b); // retourne le plus grand des deux objets
}
const Fixed &Fixed::max(const Fixed &a, const Fixed &b) // max function for const references
{
	return (a > b ? a : b); // retourne le plus grand des deux objets
}
min et max a revoir comment s'est implemente dedans
